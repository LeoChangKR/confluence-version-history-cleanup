...
personal_access_token_test = "$YOUR TOKEN$"
production_server = "https://$YOUR CONFLUENCE URL$"
test_server = "$YOUR CONFLUENCE URL$"
...
def clean_all_blog_post_version_history(space_key):
    blog_post_counter = 0
    while True:
        if len(confluence.get_all_pages_from_space(space_key, start=blog_post_counter, limit=1, status=None, expand=None, content_type='blogpost')) == 1:
            for blog_post in confluence.get_all_pages_from_space(space_key, start=blog_post_counter, limit=100, status=None, expand=None, content_type='blogpost'):
                #time.sleep(0.3)
                ...
                if confluence.history(blog_post["id"])["lastUpdated"]["number"] > 1:
                    ...
                    while True:
                        try:
                            ...
                            print("\nPossible issue in versioning. Space: {} | Blog Post: {}".format(space_key,blog_post["title"]))
                            logger.write("Possible issue in versioning. Space: {} | Blog Post: {}\n".format(space_key,blog_post["title"]))
                            break
...
def clean_all_version_history_from_all_spaces():
    ...
    while True:
        for space in confluence.get_all_spaces(start=space_counter, limit=500, expand=None)["results"]:
            ...
            clean_all_blog_post_version_history(space["key"])
        if not mod % 500 == 0:
            break
        else:
